...
public class Lector {
    ...
    public static HashMap<Integer, String> getEmpleados() throws Exception {
        ...
        datos = new AdaptadorMySQL(USER_MYSQL, PASS_MYSQL);
    ...
    public static ArrayList getMarcacionesIndividuales_() throws Exception {
        ...
        AdaptadorMySQL data = new AdaptadorMySQL(URL_MYSQL, DRIVER_MYSQL, USER_MYSQL, PASS_MYSQL);
    ...
    public static ArrayList getMarcacionesIndividuales_2() throws Exception {
        ...
        switch (tipo) {
            ...
            case DepartamentosConstantes.CONSULTA -> {
                consulta = "select hd.tarjeta as legajo, a.nombres as empleado, e.nombre as area "
                        ...
                        + "join encargados e on e.id_encargado = a.id_encargado\n"
                        + "where a.horas_extras = 1\n"
                        + "and a.activo = 1\n"
                        + "and time_to_sec(hd.horas_50)/3600 + time_to_sec(hd.horas_100)/3600 >2\n"
                        + "order by e.nombre, a.nombres";
...
//        ArrayList agentes = getAgentesM();
    ...
    public static ArrayList getDias() throws Exception {
        ...
        String consulta = "select id_dia from dias where month(id_dia) = " + Lector.MES + "";
...
//                            }
                        ...
                        irregu.add(new Irregularidad(new Agente(tarjeta, emp.get(tarjeta)), new Fecha().setFechaMySQL(Lector.ANIO + "-" + Lector.MES + "-" + dias.get(i)), 99, Irregularidad.DOS_MARCACIONES_INV));
                    }
                } else {
...
//                                System.err.println(tarjeta + "\t" + "al_50");
                        ...
                        Irregularidad irre = new Irregularidad(new Agente(tarjeta, emp.get(tarjeta)),
                                new Fecha().setFechaMySQL(Lector.ANIO + "-" + mesSt + "-" + dias.get(i)), 99, Irregularidad.DOS_MARCACIONES_INV);
...
class TDO {
    ...
    public TDO(int tarjeta, int dia, Operacion operacion) {
...
class CSI {
    ...
    public CSI(int cuenta, int suma, String id, String stPatron) {
        this.cuenta = cuenta;
        this.suma = suma;
        this.id = id;

        StringTokenizer stk = new StringTokenizer(stPatron, ",");

        ArrayList<Integer> codigos = new ArrayList();

        while (stk.hasMoreTokens()) {
            codigos.add(new Integer(stk.nextToken()));
        }

        patron = new int[codigos.size()];

        for (int i = 0; i < patron.length; i++) {
            patron[i] = codigos.get(i);
    ...
    @Override
    public String toString() {
        return cuenta + " - " + suma + " - " + id; //To change body of generated methods, choose Tools | Templates.
    }
}
...
class Verificacion {
    ...
    public Verificacion(int tarjeta, int dia, CSI csi) {
        this.tarjeta = tarjeta;
        this.dia = dia;
        this.csi = csi;
    }
